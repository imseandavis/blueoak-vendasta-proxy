...
const ContactDeleteBody = z
  .object({
    contactId: z.string().min(1), // Required for deletion
    // businessId intentionally NOT accepted from client
  })
  .strict();

type ContactDeleteBody = z.infer<typeof ContactDeleteBody>;
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const parsed = ContactDeleteBody.parse(json) as ContactDeleteBody;
