...
function assertVendastaSecrets() {
  ...
  if (!process.env.VEND_CONTACTS_UPSERT_URL) throw new Error("Missing VEND_CONTACTS_UPSERT_URL");
}
...
const ContactUpsertBody = z
  .object({
    contactId: z.string().optional(), // Optional for upsert - will create if not provided
    subtype: z.string().optional(),
    fields: z.array(Field).min(1),
    searchExisting: z.array(z.string()).optional(), // Used to find existing contact for update
    returnFields: z.array(z.string()).optional(),
    upsertStrategy: z.enum(["create_if_not_exists", "update_if_exists", "create_or_update"]).optional(),
    // businessId intentionally NOT accepted from client
  })
  .strict();

type ContactUpsertBody = z.infer<typeof ContactUpsertBody>;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = ContactUpsertBody.parse(json) as ContactUpsertBody;

    // (3) Inject businessId server-side (ignore any businessId in body if present)
    const vendPayload = { ...parsed, businessId };

    // (4) Call Vendasta
    const r = await fetch(process.env.VEND_CONTACTS_UPSERT_URL!, {
      method: "POST",
      headers: {
