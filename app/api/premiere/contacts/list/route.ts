...
function assertVendastaSecrets() {
  ...
  if (!process.env.VEND_CONTACTS_LIST_URL) throw new Error("Missing VEND_CONTACTS_LIST_URL");
}

// ---------- Zod schema (whitelist) ----------
const ContactListBody = z
  .object({
...
type ContactListBody = z.infer<typeof ContactListBody>;
...
export async function GET(req: NextRequest) {
  try {
    ...
    const parsedQuery = {
      ...
      page: queryParams.page ? JSON.parse(queryParams.page) : undefined,
      fields: queryParams.fields ? JSON.parse(queryParams.fields) : undefined,
    };

    const validated = ContactListBody.parse(parsedQuery);

    // (3) Inject businessId server-side
    const vendPayload = { ...validated, businessId };

    // (4) Build URL with query parameters
    const vendUrl = new URL(process.env.VEND_CONTACTS_LIST_URL!);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = ContactListBody.parse(json) as ContactListBody;
    ...
    const authToken = await getVendastaAuthToken();
    const r = await fetch(process.env.VEND_CONTACTS_LIST_URL!, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${authToken}`,
        "Content-Type": "application/json",
